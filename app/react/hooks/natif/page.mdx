...
Le Hook `useId` (React 18+) génère des identifiants stables et uniques, utiles pour lier des `label` à des `input` ou pour éviter les collisions d'ID lorsqu'on compose des composants réutilisables. Il est également adapté au rendu côté serveur (SSR) pour éviter les mismatches.
