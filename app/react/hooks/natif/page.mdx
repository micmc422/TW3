...
Voici des exemples fonctionnels pour chaque Hook natif de <Term word="React">React</Term> :

### 🧠 useState

Le Hook `useState` permet d'ajouter un état local à un <Term word="composant">composant</Term> fonctionnel. Il retourne un tableau avec deux éléments : la valeur actuelle de l'état et une fonction pour la mettre à jour.
   ...
   **Elle garantit que l'on travaille toujours avec la valeur la plus récente de l'état.**  
   ...
   function Counter() {
     ...
     const incrementThreeTimes = () => {
      setCount(count + 1); // ⚠️ Ne prend pas en compte les mises à jour groupées
       setCount(count + 1);
       setCount(count + 1);
     };
...
Le Hook `useEffect` permet d'effectuer des effets secondaires dans les composants fonctionnels. Il peut être utilisé pour des opérations telles que les requêtes réseau, les abonnements, les manipulations du <Term word="DOM">DOM</Term>, etc.
...
3. **Annuler une requête <Term word="HTTP">HTTP</Term> ou une WebSocket.**
...
⚠️ **Mauvais exemple (pas de nettoyage)**
```jsx copy
useEffect(() => {
...
```
➡️ **Résultat :** L’écouteur est supprimé quand le composant est démonté, **évite les bugs et les fuites mémoire**.
...
2. Il est indispensable pour :
   ...
   - **WebSockets ou requêtes <Term word="API">API</Term>** → (`abort()`)
...
Le Hook `useId` (React 18+) génère des identifiants stables et uniques, utiles pour lier des `label` à des `input` ou pour éviter les collisions d'ID lorsqu'on compose des composants réutilisables. Il est également adapté au rendu côté serveur (<Term word="SSR">SSR</Term>) pour éviter les mismatches.
...
`useTransition` permet de marquer des mises à jour d'état comme "low priority" afin que l'UI reste réactive pour les interactions prioritaires (clics, saisies rapides). `startTransition` peut être utilisé pour entourer la mise à jour coûteuse.
...
function SearchList({ items }) {
  ...
  function onChange(e) {
    ...
    setQuery(value);
    // Marque la mise à jour comme non prioritaire
    startTransition(() => {
  ...
  return (
    <div>
      <input value={query} onChange={onChange} placeholder="Rechercher..." />
...
`useDeferredValue` permet de ralentir la propagation d'une valeur au composant descendant, utile pour empêcher des recalculs coûteux à chaque frappe de l'utilisateur (par ex. recherche instantanée). Contrairement à `useTransition`, il retourne une version différée d'une valeur.
...
function Search({ items }) {
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter(i => i.includes(deferredQuery));
  }, [items, deferredQuery]);
