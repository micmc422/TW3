...
```
➡️ Ici, `useState` et `useEffect` sont appelés directement au début de la fonction, garantissant qu'ils s'exécutent dans le même ordre à chaque rendu.

---

### ⚠️ **Exemple Invalide** (Ne respecte pas les règles des Hooks)  
```jsx copy
import { useState, useEffect } from "react";
...
function InvalidComponent() {
  ...
  if (count > 5) {
  const [otherState, setOtherState] = useState("Erreur"); // ⚠️ Mauvaise pratique !
  }
...
### ⚠️ **Exemple Invalide** (Un Hook est appelé dans une fonction classique)  
```jsx copy
import { useState } from "react";

function badFunction() {
  const [count, setCount] = useState(0); // ⚠️ ERREUR : useState ne peut pas être utilisé ici
  return count;
}
...
### ⚠️ **Exemple Invalide avec une Classe** (Les Hooks ne fonctionnent pas dans les classes)  
```jsx copy
import { useState } from "react";
...
class InvalidClassComponent extends React.Component {
  ...
  render() {
  const [count, setCount] = useState(0); // ⚠️ ERREUR : Impossible d'utiliser un Hook dans une classe
    return (
      <div>
        <p>Compteur : {count}</p>
        <button onClick={() => setCount(count + 1)}>Incrémenter</button>
...
⚠️ **Les Hooks NE DOIVENT PAS être appelés dans :**
- ⚠️ Une fonction JavaScript classique (`function badFunction() {}`)
- ⚠️ Une classe React (`class MonComposant extends React.Component {}`)
