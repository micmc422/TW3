...
Voici un exemple minimaliste d'un <Term word="composant">composant</Term> fonctionnel <Term word="React">React</Term> :
...
Dans cet exemple, nous avons défini un composant fonctionnel appelé `HelloWorld`. Ce composant retourne un élément <Term word="JSX">JSX</Term> qui représente un titre de niveau 1 (`<h1>`) avec le texte "Hello, world!".
...
Note : avec le nouveau JSX transform, l'import explicite `import React from 'react'` n'est plus nécessaire dans la plupart des projets modernes (Babel/<Term word="TypeScript">TypeScript</Term>/Vite/Create React App le gèrent). Vous pouvez néanmoins l'ajouter si votre configuration le requiert.
