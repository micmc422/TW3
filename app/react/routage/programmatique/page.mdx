...
### 📦 Exemple minimal
```jsx copy filename="Redirection après formulaire"
...
### 🔁 Navigation relative & historique
```jsx copy filename="Historique"
navigate(-1); // revenir en arrière
navigate(1);  // aller en avant (si entrée disponible)
```
...
```jsx copy filename="Paramètre dynamique"
function GoUser({ id }) {
  const navigate = useNavigate();
...
```jsx copy filename="Focus après navigation"
useEffect(() => { document.getElementById('page-title')?.focus(); }, []);
```
Utiliser `tabIndex="-1"` sur le titre ciblé pour autoriser le focus.
...
## 🛠️ Exercices
1. Après login simulé, rediriger vers `/profil` avec `replace`.  
2. Ajouter un bouton « Revenir » qui désactive si historique vide (tester `window.history.length`).  
3. Passer un état `{ flash: 'Profil mis à jour' }` puis l’afficher et l’effacer.  
4. Implémenter une redirection automatique vers `/login` si token absent.
...
## ⚡ TLDR
- `useNavigate()` pour déclencher navigation
- `navigate(to, { replace, state })` pour options
- `navigate(-1)` / `navigate(1)` pour historique
- Guards : vérifier auth puis rediriger (souvent `replace: true`)
- Focus manuel après navigation pour accessibilité
