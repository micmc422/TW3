...
Avant de déployer, assurez‑vous d'avoir un script `build` dans votre `package.json`. Pour une application créée avec Create <Term word="React">React</Term> App :
...
Netlify est très simple pour les apps statiques. Dans le tableau de bord Netlify, choisissez "New site from <Term word="Git">Git</Term>", connectez votre dépôt et configurez :

- Build command : `npm run build` (ou `pnpm build` / `yarn build`)
- Publish directory : `build`

Règle importante pour les <Term word="SPA">SPA</Term> (routing côté client) : créez un fichier `_redirects` à la racine du dossier `public/` (ou dans `build/` après le build) contenant :
...
- Compression & cache : configurez headers de cache et compression (gzip/brotli) côté hébergeur ou <Term word="CDN">CDN</Term>.
