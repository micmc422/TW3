...
2. **Charge des modules <Term word="TypeScript">TypeScript</Term> dynamiques** situés dans ce chemin (ex. `seo.ts`, `cards.ts`, etc.).
...
2. **Chargement dynamique avec `import()`**
   ...
   - `seo.ts` : supposé contenir des métadonnées <Term word="SEO">SEO</Term>.
...
- `import()` est une méthode asynchrone, compatible avec ES6+ et certaines versions de <Term word="Node.js">Node.js</Term>.