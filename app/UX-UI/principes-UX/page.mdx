...
```
> 📌 `useTransition` est un hook React 19 qui permet de marquer une mise à jour comme non urgente, tout en obtenant un booléen `isPending` pour afficher un état de chargement.
...
```
> 📌 `useOptimistic` est un hook React 19 qui permet de montrer un état temporaire optimiste pendant qu'une action asynchrone est en cours.
