...
Dans ce cas pratique, nous allons explorer le fonctionnement des réseaux <Term word="Docker">Docker</Term> à travers un exemple concret de deux conteneurs Docker qui communiquent entre eux via un réseau **bridge**.
...
Docker fournit plusieurs types de réseaux, mais nous allons utiliser un réseau de type **bridge** personnalisé. Par défaut, Docker attribue à chaque <Term word="conteneur">conteneur</Term> un réseau privé isolé, mais nous allons créer un réseau commun pour que nos conteneurs puissent communiquer entre eux.
...
- **`nginx`** : Utilise l'<Term word="image">image</Term> Nginx officielle.
...
Pour tester la communication, nous allons lancer un second conteneur contenant **Curl**, un utilitaire en ligne de commande pour effectuer des requêtes <Term word="HTTP">HTTP</Term>, afin d’accéder au serveur Nginx.
