...
Dans cet exemple, nous allons empaqueter une application web simple en <Term word="Node.js">Node.js</Term> dans une <Term word="image">image</Term> <Term word="Docker">Docker</Term> afin de l'exécuter dans un <Term word="conteneur">conteneur</Term>. L'objectif est de standardiser le déploiement en encapsulant l'application et toutes ses dépendances dans une image reproductible.
...
Le **<Term word="Dockerfile">Dockerfile</Term>** est la "recette" qui permet de construire l'image Docker. Voici un exemple de Dockerfile pour notre application Node.js :
