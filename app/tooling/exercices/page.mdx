...
1. **Initialisation :**
   ...
   - Ouvrez un terminal dans ce dossier et initialisez un projet <Term word="Node.js">Node.js</Term> :
     ```bash copy
     ```
...
2. **Ajout d’outils Node.js :**
   - Installez les packages suivants :
     ```bash copy
     ```
   - Installez également <Term word="ESLint">ESLint</Term> et <Term word="Prettier">Prettier</Term> comme dépendances de développement :
     ```bash copy
...
1. **Initialisation :**
   - Initialisez un dépôt <Term word="Git">Git</Term> :
     ```bash copy
     ```
   - Créez un fichier `.gitignore` et ajoutez :
     ...
     node_modules/
     ```
   - Faites un premier <Term word="commit">commit</Term> :
     ```bash copy
