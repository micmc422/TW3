...
Les **Server Actions** permettent d’envoyer directement des données à un serveur **sans <Term word="API">API</Term> Route**, ce qui simplifie la gestion des formulaires.  
...
- La <Term word="Server Action">Server Action</Term> **`sendMessage`** est appelée directement **dans `handleSubmit`**.
...
|------------|----------------|----------------|
| 🏗️ **Complexité** | Plus simple (directement dans le <Term word="composant">composant</Term>) | Nécessite une route séparée |
