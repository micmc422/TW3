...
## **📌 2️⃣ Comment utiliser les Server Actions ?**  

### **🌍 Définir une Server Action**  

Next.js introduit le mot-clé **`"use server"`** pour déclarer une fonction exécutée côté serveur.  

🔹 **Exemple : Ajouter un commentaire à une base de données**  

🛠 **Fichier : `app/comments/page.tsx`**  
```tsx
"use client";
...
```
🛠 **Fichier : `app/actions/addComment.ts`**  
```tsx
"use server";

import { db } from "@/lib/db"; // Supposons une connexion à une base de données
...
✅ **Explication** :
- `submitComment()` appelle l’API `/actions/addComment` via `fetch()`.
- **La logique serveur est définie dans `app/actions/addComment.ts`**, non exposée au client.
- Next.js optimise la communication, **évite un appel réseau** si possible.
...
```
🛠 **Fichier : `app/actions/register.ts`**  
```tsx
"use server";
