...
<Term word="Next.js">Next.js</Term> 13+ permet une gestion native des **métadonnées sociales** (Open Graph & Twitter Cards) grâce à l'<Term word="API">API</Term> `metadata`. Ces métadonnées optimisent **le partage sur les réseaux sociaux** (Facebook, Twitter, LinkedIn, etc.), améliorant ainsi le **<Term word="SEO">SEO</Term> et la visibilité** du site.
...
- `summary_large_image` → Grande <Term word="image">image</Term> avec titre et description.  
