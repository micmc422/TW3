...
Dans <Term word="Next.js">Next.js</Term> 13+, la gestion de `robots.txt` et `sitemap.xml` est essentielle pour **le <Term word="SEO">SEO</Term> et l’indexation des pages** par les moteurs de recherche comme Google.  
...
Next.js permet de **générer dynamiquement** ces fichiers via les **routes d’<Term word="API">API</Term>** ou en utilisant le nouvel <Term word="App Router">App Router</Term>.

---

### **📂 Méthode 1 : Générer `robots.txt` avec une route `/robots.txt`**  

```tsx filename="app/robots.ts" copy
...
```tsx filename="app/sitemap.tsx" copy
import { NextResponse } from "next/server";
