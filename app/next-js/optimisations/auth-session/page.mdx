...
NextAuth.js est une solution d’authentification **simple et sécurisée** pour <Term word="Next.js">Next.js</Term>. Il prend en charge plusieurs fournisseurs (**<Term word="OAuth">OAuth</Term>, Credentials, <Term word="JWT">JWT</Term>, etc.**) et s’intègre parfaitement avec l’architecture **<Term word="App Router">App Router</Term> (`app/`)** de Next.js 13+.  
...
NextAuth.js fonctionne via une <Term word="API">API</Term> route qui gère l’authentification.  
...
- Utilise `callbacks.session` pour stocker l'ID utilisateur dans la <Term word="session">session</Term>.  
...
```tsx filename="components/AuthButton.tsx" copy
"use client";
...
Si vous voulez restreindre l’accès aux pages aux utilisateurs connectés, utilisez un **<Term word="middleware">middleware</Term>**.  
