...
Depuis <Term word="Next.js">Next.js</Term> 13, un **nouveau système de routage** appelé **<Term word="App Router">App Router</Term> (`app/`)** est introduit, en remplacement progressif du **Pages Router (`pages/`)**. Voici un comparatif détaillé entre ces deux approches, basé sur la gestion des fichiers et des fonctionnalités.  
...
✅ **<Term word="SEO">SEO</Term> et métadonnées simplifiés** avec `generateMetadata()`.  
✅ **Suppression des <Term word="API">API</Term> Routes** au profit des **Server Actions**, qui permettent d’interagir directement avec une base de données depuis un <Term word="composant">composant</Term>.  
